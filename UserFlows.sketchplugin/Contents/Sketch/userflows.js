...
var flowDirections = ['Horizontal', 'Vertical']
...
var generateFlowWithSettings = function(s) {
	...
	var exportScale = s.exportToScale,
		...
		flowVertical = getDefault('flowDirection') == 'V',
		spacing = 50*exportScale,
		outerPadding = 40*exportScale,
		selectedArtboards = [NSArray array],
		flowBoard = [MSArtboardGroup new],
		flowWidth = flowVertical ? 0 : outerPadding, 
		flowHeight = flowVertical ? outerPadding : 0,
		flowName = s.flowName,
	...
	var artboard, screenImage, screenLayer, screenFrame, aWidth, aHeight, selectionRect, hitAreaLayer, hitAreaFrame, arrowContainer, textLayer, textFrame, screenContainer, screenY, arrow, arrowStartPoint, arrowEndPoint, arrowY, arrowX, screenNumber = 0;
	...
	while (ar = [loop nextObject]) {
		...
		if(flowVertical) {
			setPosition(screenContainer, outerPadding, flowHeight)
		} else {
		...
		if(selectionRect.width != 0 && selectionRect.height != 0) {
			...
			if(flowVertical) {
				setPosition(hitAreaLayer, flowFrame.x+outerPadding+(selectionRect.x*exportScale), flowFrame.y+flowHeight+screenY+(selectionRect.y*exportScale), true)
			} else {
			...
			arrowY = flowVertical ? hitAreaFrame.y+hitAreaFrame.height : hitAreaFrame.y+(hitAreaFrame.height/2);
			arrowX = flowVertical ? hitAreaFrame.x+(hitAreaFrame.width/2) : hitAreaFrame.x+hitAreaFrame.width
			arrowStartPoint = {x:arrowX, y:arrowY}
			arrowEndPoint = flowVertical ? {x:arrowX, y:screenY+aHeight+(spacing/2)} : {x:aWidth+(spacing/2), y:arrowY}
		...
		if(flowVertical) {
			flowWidth = Math.max(flowWidth, getFrame(screenContainer).width)
			flowHeight += getFrame(screenContainer).height + spacing
		} else {
	...
	if(flowVertical) {
		flowHeight -= (spacing-outerPadding)
		flowWidth += (outerPadding*2);
	} else {
	...
	// if (flowVertical) {
	// 	setSize(flowBoard, flowWidth, flowHeight+flowInfoHeight+(outerPadding*2))
	// } else {
...
var showSettingsDialog = function() {
	...
	var lastSavedFlowDirection = getDefault('flowDirection')
	var fdIndex = lastSavedFlowDirection == 'H' ? 0 : 1
	var flowDirectionRadioButtons = createRadioButtons(flowDirections, 1, flowDirections.length, "Flow Direction", fdIndex)

	[alert addTextLabelWithValue: 'Flow Direction'] // 7
	[alert addAccessoryView: flowDirectionRadioButtons] // 8

	[alert addAccessoryView: createSeparator()] // 9
	
	[alert addTextLabelWithValue: 'Your Name'] // 10
	[alert addTextFieldWithValue: getDefault('fullName')] // 11
	
	[alert addAccessoryView: createCheckbox({name: 'Show Date and Name on Flows', value: 'showModifiedDate'}, getDefault('showModifiedDate'))] // 12
	
	[alert addAccessoryView: createSeparator()] // 13
	...
	if (response == "1000") {
		var scaleIndex = [[radioButtons selectedCell] tag]-100;

		fdIndex = [[flowDirectionRadioButtons selectedCell] tag]-100;
		var fDirection = fdIndex == 0 ? 'H' : 'V'
		...
		setDefault('flowDirection', fDirection)
		setDefault('flowIndicatorColor', newHex)
		setDefault('minimumTapArea', parseFloat(valueAtIndex(alert, 6)))
		setDefault('fullName', valueAtIndex(alert, 11))
		setDefault('showModifiedDate', checkedAtIndex(alert, 12))
